...
// Feedback do usuário (2026-08-21): a versão anterior (T8) desenhava a pista de ação DENTRO
// deste SVG (baked na imagem cacheada pelo canvas) com uma animação CSS por `currentAction` —
// dois problemas reais: (1) a chave do cache de imagem (`renderer.ts`) incluía a ação, então
// toda troca de ação recriava/decodificava uma `Image` nova por NPC, sem nunca liberar as
// antigas; (2) uma `<img>` com `@keyframes` sendo redesenhada a 60fps via `drawImage` força o
// navegador a re-rasterizar a animação a cada frame — provável causa da travada relatada. A
// pista de ação virou um overlay desenhado separadamente (`map-engine/actionIcon.ts`, canvas;
// `NpcTokenSvg.tsx`, DOM) — este SVG volta a ser só identidade, estável e cacheável por NPC.
