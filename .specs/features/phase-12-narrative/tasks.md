...
**Status**: ✅ Complete (commit 1c0d2d7) — `BeliefAssimilationService.Assimilate` resolve a crença do ouvinte via `HistoryBeliefQuery` (nunca `HistoryTruthQuery` — provado por parse Roslyn do próprio arquivo fonte), calcula confiança como `1 - DistortedReport.DistanceFromFact` (clamp `[0,1]`) e só insere em `MemoryCategory.Semantic` (`WorldState.AddNpcMemory`) quando a confiança alcança o limiar passado pelo chamador; abaixo do limiar, a exposição é só reportada no `AssimilationOutcome` retornado, sem mutar memória (NARR-13, edge case de exposição sem mutação) nem o `Fact` canônico de origem (NARR-14/15). Gate rodado como `bash scripts/test.sh --filter "FullyQualifiedName~Narrative&Category!=Scenario"` por decisão explícita do usuário nesta sessão (não `scripts/verify.sh`) — 54 passed, 0 failed.
...
**Status**: ✅ Complete (commit 6ac6698) — `NarrativeSecurityTests.cs` reusa `HistoryQuerySeparationGuard` (Fase 10, T16) para provar que nenhum host (`LivingWorld.Api`/`LivingWorld.Workers`, incluindo `NarrativeEndpoints.cs` desta fase) referencia `HistoryTruthQuery`, mais parse Roslyn por arquivo cobrindo todo `src/LivingWorld.Simulation/Narrative/*.cs` e `NarrativeEndpoints.cs` individualmente (NARR-13..15) — com teste de "scanner não é vácuo" contra fonte mutante, mesmo padrão de `ArchitectureTests`/`TruthVsBeliefBoundarySecurityTests`. Par de mutação obrigatório do `ClaimAnchorValidator` (T3, NARR-01..04): dois testes provam que um par de entrada na fronteira exata da checagem real (EventIds vazio vs não-vazio; prosa com órfão vs sem órfão) diverge de um mutante candidato explícito (`>= 0` sempre aceita; `Result.Ok` sempre sucesso) — a checagem não é tautológica. Gate rodado como `bash scripts/test.sh --filter "FullyQualifiedName~Narrative&Category!=Scenario"` por decisão explícita do usuário nesta sessão (não `scripts/verify.sh`) — 66 passed, 0 failed.
