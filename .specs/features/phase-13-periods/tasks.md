...
### T14: Campo de nome opcional em profissão/habilidade — ✅ Concluída
**What**: `ProfessionBias`/`SkillBias`/`PeriodCatalogResponse` ganham um campo `Name` opcional (`string?`) — motor nunca decide por ele (só armazena/devolve), preenchido pela IA externa/documentação quando quiser nomear. Cobre a lacuna das ACs "id + nome opcional" (habilidade aberta) e "nomes quando declarados" (leitura de catálogo) que hoje não têm nenhum campo de nome em lugar nenhum. Design: `Name` vira 3º parâmetro opcional (`= null`) de `ProfessionBias`/`SkillBias` (parse tolera ausência, rejeita tipo não-string nomeando o campo); `PeriodCatalog`/`PeriodCatalogResponse` ganham `ProfessionNames`/`SkillNames` como `IReadOnlyDictionary<int,string>` — só contém o id que declarou nome num bias, nunca todo o catálogo — aditivo, não alterou `ProfessionIds`/`SkillIds` existentes. **Where**: `src/LivingWorld.Simulation/Periods/PeriodDynamicsLoader.cs`, `PeriodCatalog.cs`, `src/LivingWorld.Api/PeriodsEndpoints.cs`. **Depends on**: T12. **Requirement**: PERIOD-19, PERIOD-22..23. **Tests**: unit (`PeriodDynamicsLoaderTests`, `PeriodCatalogTests`) + integration (`PeriodsEndpointTests`). **Gate**: Quick — `bash scripts/test.sh --filter Category!=Scenario`: 1108 passed, 0 failed (11 skipped = baselines `ZZZ_record_*`, esperado).

### T15: Teste de arquitetura pra literal de nome de habilidade — ✅ Concluída
**What**: mesmo padrão de `PeriodArchitectureTests`/`PopulationArchitectureTests` — reprova qualquer nome de habilidade banido virando literal de decisão em `LivingWorld.Domain`/`LivingWorld.Simulation`. AC explícita da story "Habilidade como catálogo aberto" que não tinha teste. Lista banida = os 13 nomes do antigo enum fechado `SkillType` (Fase 6, removido em T11b): Agriculture, Hunting, Trade, Construction, Medicine, Combat, Teaching, Craft, Politics, Leadership, Research, Technology, Magic. **Where**: `tests/LivingWorld.Tests/Periods/SkillArchitectureTests.cs` (novo). **Depends on**: T12. **Requirement**: PERIOD-20. **Tests**: architecture. **Gate**: Quick — `bash scripts/test.sh --filter Category!=Scenario`: 1110 passed, 0 failed (11 skipped = baselines, esperado).

### T16: Corrigir `period-authoring.md` sobre transformação em runtime — ✅ Concluída
**What**: o índice (`docs/domain/period-authoring.md`) afirma que regras de transformação agem "ao longo da simulação" — impreciso antes de T13. Depois de T13, revisar a frase pra refletir o comportamento real (`TriggerTick`, reatribuição, idempotência). Frase revisada: cada regra dispara uma única vez no tick em que `TriggerTick` é alcançado (ou desde o tick 0 se ausente), reatribui os NPCs afetados e nunca reaplica (idempotente por pertença ao catálogo, sem estado extra no mundo) — reflete `PeriodEvolutionSystem` (T13). **Where**: `docs/domain/period-authoring.md`. **Depends on**: T13. **Requirement**: — (qualidade de documentação). **Tests**: none. **Gate**: Build — `bash scripts/build.sh`: 0 warnings/erros.
