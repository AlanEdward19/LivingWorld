...
| Animação/movimento no mapa (doc#132-133) | Fora do P1 desta demo — mapa é interativo (clique/zoom/pan) mas sem animação de NPCs se movendo, caravanas viajando etc. | Não foi perguntado ao usuário; animação é polish visual (doc#142 ordem "10. Animação" é o item MENOS prioritário da lista) — não bloqueia validar a hierarquia de informação, que é o objetivo real desta demo | n — assumido |
...
1. WHEN o zoom está no nível "mundo" THEN o mapa SHALL mostrar só assentamentos, rótulos, e (quando aplicável ao fixture) eventos importantes marcados — nenhum NPC individual visível (doc#106).
2. WHEN o usuário dá zoom até o nível "distrito/assentamento" (dentro de Oakbridge) THEN o mapa SHALL mostrar edifícios importantes e áreas — ainda sem NPCs individuais posicionados (doc#106).
3. WHEN o usuário dá zoom até o nível "agente" THEN o mapa SHALL mostrar NPCs individuais posicionados (token de NPC reusado, AC-fixo desta spec) — pelo menos Mira, e os demais membros do household/relacionamentos do fixture — clicáveis, abrindo o Agent View ao clicar (doc#106).
