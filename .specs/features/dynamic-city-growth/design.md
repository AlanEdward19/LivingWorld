...
| --------------- | -------- | ------------ |
| Whole map has no free cell reachable by the ring search (fully built world) | `BuildingPlacementResolver.Resolve` itself never queues or retries anything — it returns `null` immediately, every call, when the ring search (map-clamped) finds nothing. Land scarcity is fed into the **existing** `MigrationSystem` daily scoring instead of a new mechanism: `MigrationSystem.ScoreOf` already computes a "stay in current city" score vs. every other city's score (employment/food/security/family-ties weights, see file header). When a city has zero free cells anywhere on the map, its own "stay" score for every household there is forced to the theoretical minimum — the household's *existing* daily migration decision naturally picks a candidate city over staying. If `world.Cities.Count < 2` (nowhere to go), `MigrationSystem.Tick` already no-ops. | Household relocates to an existing city with room, exactly like any other migration — no visible new mechanic. |
| `ConstructionSystem`'s pre-existing (Fase 8) FIFO project completes payment the same tick `Resolve` returns `null` for it | **(AD-007, amends round-1 wording)** The project is NOT dropped (would destroy its consumed resources/orphan nothing to show for them) and is NOT left blocking the queue head forever either (would starve every other project behind it — measured 20+ ticks in testing). `ConstructionSystem.Tick` skips a stuck (fully-paid, unplaceable) project for that tick's resource-consumption budget and retries its placement every tick at zero resource cost, while advancing the first NOT-yet-stuck project behind it as the tick's actual resource-consuming "head" — same one-project-per-tick resource throttle as before, just no longer strictly index-0-only. | A land-scarce city keeps building everything else normally; the stuck project completes automatically the first tick land becomes available (e.g. after a migration/founding event changes the map), with its resources charged exactly once, never twice. |
